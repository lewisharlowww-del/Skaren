...
type BadgeCardProps = {
  ...
  earned: boolean;
  icon: LucideIcon;
  /** Localized status labels — passed in so the card carries no English of its own. */
  earnedLabel?: string;
  lockedLabel?: string;
};

export function BadgeCard({ name, earned, icon: Icon, earnedLabel = "Earned", lockedLabel = "Locked" }: BadgeCardProps) {
  return (
