...
export function StatCard({ label, value, icon: Icon, detail, tone = "green" }: StatCardProps) {
  ...
  return (
    <div className={`min-w-0 rounded-[1.75rem] border border-white/70 bg-gradient-to-br p-5 shadow-soft backdrop-blur-xl ${tones[tone]}`}>
      ...
      <p className={`type-section-label ${dark ? "text-white/55" : "text-soil-500"}`}>{label}</p>
      <p className={`type-heading-1 mt-2 truncate ${dark ? "text-white" : "text-ink"}`}>{value}</p>
      {detail ? <p className={`type-body-sm mt-2 line-clamp-2 ${dark ? "text-white/65" : "text-soil-600"}`}>{detail}</p> : null}
    </div>
  );
}
