...
export function StatCard({ label, value, icon: Icon, detail, tone = "green" }: StatCardProps) {
  ...
  return (
    <div className={`rounded-[1.75rem] border border-white/70 bg-gradient-to-br p-5 shadow-soft backdrop-blur-xl ${tones[tone]}`}>
      ...
      <p className={`mt-2 font-display text-3xl font-black tracking-[-0.04em] ${dark ? "text-white" : "text-ink"}`}>{value}</p>
